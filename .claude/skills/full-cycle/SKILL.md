...
# 체크포인트마다 확인 요청 (제어권 강화)
/full-cycle #12 --interactive

# 특정 Phase에서 중단
/full-cycle #12 --stop-at phase2
```
...
### 기본 동작 (자동 진행)
각 Phase 완료 시 결과 출력 후 자동으로 다음 Phase 진행
...
- 여러 이슈 동시 처리 (한 번에 하나씩)
- Quality Gate 실패 무시하고 PR 생성
- main 브랜치로 직접 PR
- rebase 없이 push
- 테스트 없이 Service 코드 커밋
- /code-review 치명적 이슈 무시
- Phase 건너뛰기 (순서대로 진행)
- 이슈 내용 확인 없이 구현 시작
